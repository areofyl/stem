...
class BandMergeModule(nn.Module):
    ...
    def forward(self, x, orig_freq):
        ...
        # project each band and cat along freq axis (no scatter/slice assign)
        band_outputs = []
        for i, (start, end) in enumerate(self.bands):
            raw = self.projections[i](x[:, i])  # (B, T, band_size * C * 2 * n_sources)
            band_size = end - start
            raw = raw.reshape(B, T, self.n_sources, self.n_channels, 2, band_size)
            raw = raw.permute(0, 2, 3, 4, 5, 1)  # (B, sources, C, 2, band_size, T)
            band_outputs.append(raw)

        output = torch.cat(band_outputs, dim=4)  # cat along freq axis
...
class StemSeparator(nn.Module):
    ...
    def stft_real(self, x):
        """STFT that returns (real, imag) as separate tensors, no complex"""
        # x: (B, samples)
        win = self._get_window(x.device)
        spec = torch.stft(x, self.n_fft, self.hop_length,
                         window=win, return_complex=False)
        # spec: (B, freq, time, 2) where last dim is [real, imag]
        return spec[..., 0], spec[..., 1]  # (B, freq, time) each

    def istft_real(self, real, imag, length):
        """ISTFT from real/imag without complex tensors.
        manual overlap-add: mirror spectrum, IDFT via IRFFT trick, window, sum."""
        win = self._get_window(real.device)
        # real, imag: (B, freq, time)
        B, F, T = real.shape
        hop = self.hop_length
        n_fft = self.n_fft

        # reconstruct full spectrum: mirror conjugate for bins above nyquist
        # full_real: (B, n_fft, T), full_imag: (B, n_fft, T)
        full_real = torch.cat([real, real[:, -2:0:-1, :]], dim=1)
        full_imag = torch.cat([imag, -imag[:, -2:0:-1, :]], dim=1)

        # IDFT each frame: use the identity that IFFT of (re,im) pairs
        # can be computed via FFT of (re, -im) then divide by N
        # but that still uses complex... let's just use the overlap-add formula directly
        # actually torch.fft.irfft works on real tensors if we construct it right
        spec = torch.complex(real, imag)
        frames = torch.fft.irfft(spec, n=n_fft, dim=1)  # (B, n_fft, T)

        # overlap-add
        output = torch.zeros(B, length, device=real.device)
        window_sum = torch.zeros(length, device=real.device)

        for t in range(T):
            start = t * hop
            end = min(start + n_fft, length)
            seg_len = end - start
            output[:, start:end] += frames[:, :seg_len, t] * win[:seg_len]
            window_sum[start:end] += win[:seg_len] ** 2

        window_sum = window_sum.clamp(min=1e-8)
    ...
    def forward_chunk(self, mix):
        ...
        # STFT both channels into real/imag pairs
        specs_r = []  # real parts
        specs_i = []  # imag parts
        for ch in range(C):
            r, i = self.stft_real(mix[:, ch])
            specs_r.append(r)
            specs_i.append(i)

        freq = specs_r[0].shape[1]

        # stack into (B, channels, 2, freq, time) for band split
        spec_ri = torch.stack([
            torch.stack(specs_r, dim=1),  # (B, channels, freq, time)
            torch.stack(specs_i, dim=1),
        ], dim=2)
        ...
        # apply masks using real arithmetic: (a+bi)(c+di) = (ac-bd) + (ad+bc)i
        sources = []
        for s in range(self.n_sources):
            ...
            for ch in range(C):
                ...
                out_r = orig_r * mask_r - orig_i * mask_i
                out_i = orig_r * mask_i + orig_i * mask_r

                audio = self.istft_real(out_r, out_i, L)
                source_channels.append(audio)
            sources.append(torch.stack(source_channels, dim=1))
